...
class binPartitionIssue : AppCompatActivity() {
    data class BinInfo(val id: Int,
    ...
    private lateinit var binNum:EditText
    private lateinit var currentContext:Context
    private lateinit var infoTV:TextView
    private lateinit var btnSave:Button
    private lateinit var binData:BinInfo
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_bin_partition_issue)
        ...
        currentContext = this@binPartitionIssue
        infoTV = findViewById(R.id.BPITVInfo)
        btnSave = findViewById(R.id.BPISave)
        binData = BinInfo(id = 0,binNumber = "",type = "",
            dateRegistered = "",pic = "",status = "")
        binNum = findViewById(R.id.BPIEDBinNo)
        binNum.setOnKeyListener(View.OnKeyListener { _, keyCode, event ->
            if(keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_DOWN ||
                ...
                keyCode == KeyEvent.KEYCODE_TAB && event.action == KeyEvent.ACTION_UP){

                CoroutineScope(Dispatchers.IO).launch {
                    retrieveBinInfo(binNum.text.toString())
                }
        ...
        btnSave.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                if(binNum.text.toString() == "" || binNum.text.toString().isNullOrBlank()) {
                    if(infoTV.text != ""){
        ...
        withContext(Dispatchers.IO){
            ...
            try{
                ...
                if(responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    binData = BinInfo(id=binId,binNumber=binNumb,type=typ,dateRegistered=date_Registered,
                        pic = binPic, status = binStatus)
                }
                else{
                ...
                if (e.message.toString() == "Index 0 out of range [0..0)"){
                    TriggerAlert("BIN ${binNo}, Not register","Error")
                }
                else{
    ...
    private fun resetForm(){
        runOnUiThread(kotlinx.coroutines.Runnable {
            infoTV.setText("")
            binData = BinInfo(id = 0,binNumber = "",type = "",dateRegistered = "",pic = "",status = "")
            binNum.text.clear()
        })
